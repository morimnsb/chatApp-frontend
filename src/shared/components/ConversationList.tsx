...
export default function ConversationList({
  ...
  return (
    <Gate loading={loading} error={error} onRetry={onRetry}>
      <ListGroup className="message-list-wrapper">
        ...
        {individualMessages.length ? (
          individualMessages.map((convo: any) => {
            ...
            const inlinePreview = !incoming && !outgoing ? clip(lastMsgText, 28) : "";
            ...
            return (
              <ListGroup.Item key={`dm-${roomId || userId}`} className={`message-list-item p-0 ${isActive ? "active" : ""}`}>
                <div
                  ...
                  onKeyDown={(e) => e.key === "Enter" && handleSelectChat(roomId, userId)}
                >
            ...
            return (
              <ListGroup.Item key={`group-${roomId}`} className={`message-list-item p-0 ${isActive ? "active" : ""}`}>
                <div
                  ...
                  onKeyDown={(e) => e.key === "Enter" && handleSelectChat(roomId)}
                >
                  <div className="message-content">
