// chatApp-frontend\src\features\auth\components\RegisterForm.tsx
...
// اگر authService تایپ برنگردونه، این رو نرم نگه می‌داریم
type RegisterResponse = {
  otp?: string | number;
  [k: string]: any;
};

const normalize = (s: unknown): string => String(s || "").trim();
const normalizeSpace = (s: unknown): string => normalize(s).replace(/\s+/g, " ");
...
function pickServerMessage(err: unknown): string {
  ...
  } else if (e?.message === "NO_XSRF_TOKEN") {
...
const RegisterForm: React.FC = () => {
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData((prev) => ({
      ...prev,
      // name حتماً یکی از کلیدهای formData هست
      [name]: value,
    }));
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const data = (await registerApi(payload)) as RegisterResponse;

      console.debug("[REGISTER RESPONSE DATA]", data);

      // ✅ Keep verify-email flow
      sessionStorage.setItem("pending_email", normalize(email));
      if (data?.otp != null) sessionStorage.setItem("pending_otp", String(data.otp));
      ...
      const message = pickServerMessage(err);
